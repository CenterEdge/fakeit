...
module.exports = is.object({
  _id: is.string().pattern(/airport_[0-9]+/),
  airport_id: is.number(),
  doc_type: 'airport',
  airport_ident: [ is.string().uppercase(), null ],
  airport_type: [ is.string().pattern(/^[a-z][a-z_]+$/), null ],
  ...
  iso_continent: is.string().pattern(/^[A-Z]{2}$/),
  iso_country: is.string().pattern(/^[A-Z]{2}$/),
  ...
  timezone: [ is.string().pattern(/^[A-Z][A-Za-z_\s]+\/[A-Z][A-Za-z_\s]+$/), null ],
});
